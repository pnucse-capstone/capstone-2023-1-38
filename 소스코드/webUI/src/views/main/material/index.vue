<template>
	<div class="app-container">
		...
		<el-table :data="transportList">
			...
			<el-table-column label="농가" prop="farmerNickName" />
			<el-table-column label="원료 소속" prop="farmerDept" />
			<el-table-column label="시간" prop="time" />
			<el-table-column label="상태" prop="outFactoryStatus">
				<template slot-scope="scope">
					<el-tag v-show="scope.row.outFactoryStatus === 0">입고됨</el-tag>
			...
			<el-table-column label="조작" align="center" class-name="small-padding fixed-width">
				<template slot-scope="scope">
					<el-button v-show="scope.row.outFactoryStatus === 0" size="mini" type="text" @click="noticeTransport(scope.row)">운송 연락(1)</el-button>
          <el-button v-show="scope.row.outFactoryStatus === 0" size="mini" type="text" @click="outFactory(scope.row)">출고(2)</el-button>
        </template>
			</el-table-column>
		</el-table>
		...
		<el-dialog center title="운송 연락" :visible.sync="noticeDetaiDialog" width="500px" append-to-body>
			<el-form ref="form" :model="trasportForm" label-width="80px">
				...
				<el-form-item label="비고"><el-input v-model="trasportForm.remarks" type="textarea" placeholder="내용을 입력하세요"></el-input></el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
